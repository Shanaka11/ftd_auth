    ...
    try:
        with transaction.atomic():
            if User.objects.filter(username=data['email']).exists():
                ...
                activation_link = settings.C_CLIENT_URL + "/user/validate?id=" + str(user.id) + "&token=" + str(token)
                context = {
...
def VerifyEmail(request, user_id=None, token=None):
    ...
    try:
        ...
        decode_token = jwt.decode(byte_token, settings.C_JWT_KEY, algorithms='HS256')
        # if token is valid then do the activate the user
        if (datetime.datetime.now() - datetime.datetime.utcfromtimestamp(0)).total_seconds() - decode_token["exp"] <= settings.C_JWT_TOKEN_EXP:
...
def ChangePasswordRequest(request):
    try:
        ...
        token = jwt.encode({'name': user.first_name, 'exp': datetime.datetime.now()}, settings.C_JWT_KEY, algorithm='HS256').decode() 
        activation_link = settings.C_CLIENT_URL + "user/password_reset?id=" + str(user.id) + "&token=" + str(token)
        context = {
            "name": user.first_name,
            "link": activation_link
        }
